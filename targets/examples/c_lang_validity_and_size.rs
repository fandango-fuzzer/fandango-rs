//! See how many generated C programs compile with gcc, using validity constrains + generation goals + grammar.
...
fn main_repl() -> Result<(f32, i32, i32, i32, f32, f32), Error> {
    ...
    loop {
        let (generated, fitness_1, accepted, elapsed_gen, elapsed_compile) =
            run_once(&mut all_compiling_programs, false, false, false)?;
...
fn main() -> Result<(), Error> {
    ...
    println!(
        "\\newcommand{{\\validAndSizedConstrainedTotalTimeAvgRs}}{{{:.2}s\\xspace}}",
        average_elapsed_gen + average_elapsed_compile
    );

    // Also write these macros to a .txt file
    let mut file = std::fs::File::create("c_lang_validity_and_size_results.txt")?;
    use std::io::Write;
    writeln!(
        file,
        "\\newcommand{{\\validAndSizedConstrainedKPathCoverageAvgRs}}{{{:.2}\\%\\xspace}}",
