...
fn main() -> Result<(), Error> {
    ...
    println!("Averages over {num_runs} runs:");

    println!(
        "\\newcommand{{\\{name}KPathCoverageAvgRs}}{{{average_covered_percentage:.2}\\%\\xspace}}"
    );

    println!("\\newcommand{{\\{name}TotalAvgRs}}{{{average_programs_generated:.2}\\xspace}}");

    println!("\\newcommand{{\\{name}FitOneAvgRs}}{{{average_programs_with_fitness_1:.2}\\xspace}}");

    println!(
        "\\newcommand{{\\{name}CompileAvgRs}}{{{average_programs_accepted_by_gcc:.2}\\xspace}}"
    );

    println!("\\newcommand{{\\{name}GenTimeAvgRs}}{{{average_elapsed_gen:.2}s\\xspace}}");

    println!("\\newcommand{{\\{name}CompileTimeAvgRs}}{{{average_elapsed_compile:.2}s\\xspace}}");
