//! See how many generated C programs compile with gcc, using only validity constrains + grammar.
...
fn main() -> Result<(), Error> {
    ...
    println!(
        "\\newcommand{{\\validConstrainedTotalTimeAvgRs}}{{{:.2}s\\xspace}}",
        average_elapsed_gen + average_elapsed_compile
    );

    // Also write these macros to a .txt file
    let mut file = std::fs::File::create("c_lang_validity_only_results.txt")?;
    use std::io::Write;
    writeln!(
        file,
        "\\newcommand{{\\validConstrainedKPathCoverageAvgRs}}{{{:.2}\\%\\xspace}}",
